...
export function updateShell(name) {
  ...
  const labels = personal ? { overview: 'Spending', expenses: 'Transactions', study: 'Spending details', settings: 'Settings' } : pages;
  ...
  for (const button of document.querySelectorAll('.tabs button')) {
    button.hidden = !Object.hasOwn(labels, button.dataset.tab) || (personal && button.dataset.tab === 'study');
    ...
    if (button.dataset.tab === (personal && name === 'study' ? 'overview' : name)) button.setAttribute('aria-current', 'page');
    else button.removeAttribute('aria-current');
  }
  // A single navigation order for each workspace, including its phone layout.
  const order = personal ? ['overview', 'expenses', 'settings'] : ['overview', 'week', 'expenses', 'settings'];
