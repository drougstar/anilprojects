...
const claimChoices = () => personal() ? [['personal', 'Personal']] : [['business', 'Claim from company'], ['personal', 'Not claimed']];
const claimHelp = 'Not claimed stays with this Work sheet or trip, outside IFS. It also appears in Personal’s All spending view. Import bank statements in Personal.';
...
export function openExpenseTools(row = null) {
  ...
  if (row) {
    ...
    add('Split receipt', personal() ? 'Divide the exact total into separate spending entries.' : 'Divide the receipt between company claims, not-claimed costs or projects.', () => openSplit(row));
...
export async function openReimbursements({ ids } = {}) {
  ...
  const host = el('div', { class: 'workflow-list reimbursement-list' }), mode = select([['open', 'Awaiting payment'], ['all', 'All company claims'], ['paid', 'Fully paid']], 'open');
...
export function openSplit(row) {
  ...
  function addPart(amount = '', business = !personal()) {
    ...
    const who = select(claimChoices(), business ? 'business' : 'personal', { 'aria-label': 'Split type' });
    const shortName = input(row.shortName || '', 'text', { placeholder: 'Project short name (company claims)', 'aria-label': 'Split project' });
    ...
    const node = el('div', { class: 'workflow-split-row' }, field(`Amount (${row.currency})`, amountIn), personal() ? field('Category', category) : field('Company claim', who), field(personal() ? 'Note' : 'Description', written), personal() ? null : field('Project', shortName), el('button', { class: 'link', onclick: () => { parts.splice(parts.indexOf(part), 1); node.remove(); showTotal(); } }, 'Remove'));
  ...
  const d = openDialog('Split receipt', el('div', { class: 'form' }, el('p', {}, `Original: ${expenseLabel(row)}`), el('p', { class: 'help' }, 'The original is replaced by these parts in one save. They keep its date, currency and photos. History can undo the whole split.'), personal() ? null : el('p', { class: 'help' }, claimHelp), partsHost, remaining,
...
export async function openInboxReview(item) {
  ...
  const who = select(claimChoices(), personal() || draft.business === false ? 'personal' : 'business');
  ...
  const d = openDialog('Review receipt', el('div', { class: 'form' }, image,
    ...
    el('div', { class: 'grid2' }, field('Amount', amount), field('Currency', cur), field('Date', date), field(personal() ? 'Category' : 'Type', personal() ? category : code)), field(personal() ? 'Note' : 'Description', written), personal() ? field('Merchant', merchant) : null, personal() ? null : field('Company claim', who, claimHelp),
...
export function openTemplate(template = null, row = null) {
  ...
  const name = input(template?.name || row?.written || ''), amount = input(defaults.amount, 'number', { step: '0.01' }), cur = currencySelect(defaults.currency), written = input(defaults.written || ''), code = select(ctx.settings().expenseCodes.map(c => [c.code, c.short]), defaults.code || defaultCode()), who = select(claimChoices(), defaults.business && !personal() ? 'business' : 'personal');
